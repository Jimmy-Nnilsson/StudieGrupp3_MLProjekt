...
class BEX_aug_geometric():
    ...
    def aug_image_and_save_to_dict(self, images, list_of_names):
        ...
        dictionary = {}
        list_of_angles = [0, 90, 180, 270]                                  #hur vi vill rotera bilderna
        
        for k in range(len(list_of_names)):
            for i in list_of_angles:                                        #roterar bilden 4 gånger 
                transform = A.Compose([A.Affine(scale=1, 
                                        ...
                                        always_apply=False, 
                                        p=1)])
                augmented_image = transform(image=images[k])['image']       #exekverar funktionen Affine (enbart rotate)
                augmented_image = Image.fromarray(augmented_image)          #konverterar bild till array
                try:                                                        
                    dictionary[list_of_names[k]] += [augmented_image]       #sparar sedan filerna i en lista inuti dict
                except:
                    dictionary[list_of_names[k]] = [augmented_image]

            for i in range(len(dictionary[list_of_names[k]])):              #tar de fyra roterade bilderna och flippar dem
            ...
            for i in range(len(dictionary[list_of_names[k]])):              #tar de roterade och flippade bilderna och lägger på 1.3 gånger zoom        
                transform = A.Compose([A.Affine(scale=1.3, 
                                        translate_percent=None, 
                                        translate_px=None, 
                                        rotate=0, 
                                        shear=None, 
                ...
                augmented_image = transform(image=zoom)['image']           #exekverar funktionen Affine(enbart scale används)
                augmented_image = Image.fromarray(augmented_image)         #konverterar bild till array
                dictionary[list_of_names[k]] += [augmented_image]          #sparar i dict
        return dictionary
